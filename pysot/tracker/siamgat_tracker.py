...
class SiamGATTracker(SiameseTracker):
    ...
    def init2(self,img,bbox):
        ...
        self.model.template2(z_crop, roi)

    def per_init(self, img, bbox):
        """
        args:
            img(np.ndarray): BGR image
            bbox: (x, y, w, h) bbox
    ...
    def track(self, img, pre_img, pre_box, init_img, init_bbox, similarity, idx):
        """
        args:
            img(np.ndarray): BGR image
        return:
        ...
        # 改
        outputs = self.model.track(x_crop, pre_img, pre_box, init_img, init_bbox, similarity, idx)
